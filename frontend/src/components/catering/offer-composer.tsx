...
import { ChevronLeft, ImagePlus, Loader2, UtensilsCrossed, X } from 'lucide-react'
import { CREATE_CATERING_OFFER_MUTATION } from '@/graphql/catering/mutations'
...
interface OfferComposerProps {
  /** The catering page the offer is published under (you must own it). */
  pageId: string
  /** Called after an offer is created so the parent can refresh its list. */
  onCreated?: () => void
}

export function OfferComposer({ pageId, onCreated }: OfferComposerProps) {
  ...
  const [createOffer, { loading: creating }] = useMutation(
    ...
    void createOffer({
      variables: {
        input: {
          ...
          pricePerPerson: Number.isNaN(parsedPrice) ? null : parsedPrice,
          imageUrls,
        },
      },
    })
    ...
    return (
      <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {CATERING_KINDS.map(({ id, icon: Icon }) => (
            <button
              key={id}
              type="button"
              onClick={() => setKind(id)}
  ...
  const KindIcon = cateringKindIcon(kind)
  ...
  return (
    ...
    >
      ...
      {/* Price per person */}
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="offer-price">{t('price')}</Label>
        <Input
          id="offer-price"
          type="number"
          min={0}
